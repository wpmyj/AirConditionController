...
static void halInternalInitUartInterrupts(uint8_t port)
{
    ...
    if (EM_SER_MULTI(EM_SER1_PORT_FIFO(port) || EM_SER2_PORT_FIFO(port)))
    {
        ...
        INT_SCxCFG(port) |= (INT_SCRXVAL | INT_SCRXOVF | INT_SC1FRMERR
                             | INT_SC1PARERR);
...
static EmberStatus halInternalInitPhysicalUart(uint8_t port,
        SerialBaudRate rate, SerialParity parity, uint8_t stopBits)
{
    uint32_t tempcfg;
...
static EmberStatus halInternalInitUsbVcp(void)
{
    ...
    return EMBER_SERIAL_INVALID_PORT;
     ...
     EM_SERIAL2_ENABLED ||\
     EM_SERIAL3_ENABLED)
EmberStatus halInternalUartInit(uint8_t port, SerialBaudRate rate,
                                SerialParity parity, uint8_t stopBits)
{
#if EM_SERIAL0_ENABLED
...
EmberStatus halInternalForceWriteUartData(uint8_t port, uint8_t *data,
        uint8_t length)
{
#if EM_SERIAL0_ENABLED
    if (EM_SER0_PORT_EN(port))
...
//USART0 接收器缓冲
#define  RX_BUFFER_SIZE0    26
unsigned char rx_buffer0[RX_BUFFER_SIZE0];
// USART0 接收中断服务程序
unsigned char Check_Rx_Data_Flag_0 =0x00;     //接收数据完毕 检查接收数据标志
unsigned char Rx_Data_Start_Flag_0 =0x00;     //接收有效数据开始标志 

unsigned char Rx_Data_Start_SIZE_buffer;//接收有效数据为多少字节
unsigned char USART_Tx_Rx_Count_0;      //发送或接收 --数据计数器
unsigned char Synchronous_marking;
uint16_t        BUFFER_SIZE0;             //用于接收数据的字节数
uint8_t         rxd_data;

extern EmberEventControl UARTEventControl;
void halInternalUartRxIsr(uint8_t port, uint16_t causes)
{
//------------------------------------------------------------------------------------------------------------------------
    while ( SCx_REG(port, UARTSTAT) & SC_UARTRXVAL )
    {
        rxd_data = (uint8_t) SCx_REG(port, DATA);//////////////////////////////////
        rx_buffer0[USART_Tx_Rx_Count_0] = rxd_data;
        USART_Tx_Rx_Count_0++;
		USART_Tx_Rx_Count_0 = USART_Tx_Rx_Count_0 % RX_BUFFER_SIZE0;
    }
//------------------------------------------------------------------------------------------------------------------------
    ...
    if (EM_SER_MULTI(EM_SER1_PORT_FIFO(port) || EM_SER2_PORT_FIFO(port)))
    {
        ...
        while (SCx_REG(port, UARTSTAT) & SC_UARTRXVAL)
        {
            uint8_t errors = SCx_REG(port, UARTSTAT)
                             & (SC_UARTFRMERR | SC_UARTRXOVF | SC_UARTPARERR);
...
static void halInternalUartTxIsr(uint8_t port)
{
    ...
    if (EM_SER_MULTI(EM_SER1_PORT_BUFFER(port) ||EM_SER2_PORT_BUFFER(port)))
    {
        ...
        while ( q->used > 0 )
        {
            ...
            else if ( !(SCx_REG(port, DMACTRL) & SC_TXLODB) &&
