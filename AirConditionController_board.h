...
#define MAX_LED_NUMBER 1			//add by sven
...
// BUTTON_INT ?????????━?1?D??IO(?????━???????━??D?????????━?112????━???????━???????D??IO????′?o???????━???a????━??D????━???????????????━?1?????????IO2????━?2????━?|????━?2?D??|???′?????),????━?????━???????o????━?o?????━???????━?o1????━??????????━o???━??━????：???：??????━?o???|????━????：???：?????━?a?????????━?1
#define BUTTON_INT          PORTB_PIN(6)
...
/**
 * @brief The GPIO input register for BUTTON_INT.
 */
#define BUTTON_INT_IN       GPIO_PBIN
/**
 * @brief Point the proper IRQ at the desired pin for BUTTON_INT.
 * @note IRQA is fixed and as such does not need any selection operation.
 */
#define BUTTON_INT_SEL()    do { } while(0)
/**
 * @brief The interrupt service routine for BUTTON_INT.
 */
#define BUTTON_INT_ISR         halIrqBIsr
/**
 * @brief The interrupt configuration register for BUTTON_INT.
 */
#define BUTTON_INT_INTCFG      GPIO_INTCFGB
/**
 * @brief The interrupt enable bit for BUTTON_INT.
 */
#define BUTTON_INT_EN_BIT       INT_IRQB
/**
 * @brief The interrupt flag bit for BUTTON_INT.
 */
#define BUTTON_INT_FLAG_BIT    INT_IRQBFLAG
/**
 * @brief The missed interrupt bit for BUTTON_INT.
 */
#define BUTTON_INT_MISS_BIT    INT_MISSIRQB
